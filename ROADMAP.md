...
| N · cross-domain analogy | ✅ this PR | the SME mechanism **across two literatures**: a declared role ascension lets `CAUSE(premise→guarantee)` align across domains, surfacing *weighted-conformal ~~ Banach-contraction* (`cal_test↔the_map`, `interval↔iterate-seq`); `analogous_to` edges in the same graph; lineage shown domain-agnostic |
