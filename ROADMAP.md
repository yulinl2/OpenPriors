...
| T · top-level legibility | ✅ this PR | root **README + Makefile** present the full A→S story as one system; `make setup && make pipeline && make query && make test` — the repo reads top-to-bottom and the capstone runs in one command |
