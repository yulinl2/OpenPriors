...
| R · capstone pipeline | ✅ this PR | **the whole thesis as one command**: ingest→lineage→unify→discover analogies→transfer→evaluate over three literatures, one summary + one unified graph (197 nodes incl. results/facts/functors/entities/conjectures, 367 edges). End-to-end invariants gated in CI |
