...
| 🎈 interactive demo | ✅ this PR | the **public front door**: a single self-contained `docs/index.html` (GitHub Pages) with four zoom levels — Story → Map → Discoveries → Under-the-Hood — built live from the pipeline output (`make demo`), so it never drifts. `demo/model.py` + `demo/build_demo.py`; demo-ci gates it in sync + self-contained |
