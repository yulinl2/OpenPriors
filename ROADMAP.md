...
| U · fourth literature | ✅ this PR | **martingale concentration** (McDiarmid/Bernstein) joins the analogy web with **zero new design**: its `BOUNDED_MARTINGALE` is auto-discovered as the same `PC/2` structural-property role → the **4-way** analogy *…:: bounded-martingale : concentration*. Pipeline now 4 literatures / 278 nodes / 24 analogies; lineage `bernstein → mcdiarmid` |
