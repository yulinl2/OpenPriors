...
| AH · fourth experiment (C7) | ✅ this PR | **every committed research direction is now computed** (C2, C4, C5, C7). C7's own proposed test: quadratics `f_t(x)=½(x−c_t)ᵀdiag(1,κ)(x−c_t)` across three regimes. Confirmed: the informed step `η=1/L` contracts geometrically at **exactly `1−1/κ`** (machine-eps by T) while the regret-optimal schedule `η_t=min(1/L,1/μt)` — which earns **stable regret/log T** — decays only **polynomially (exponent −1)**: a super-geometric **price of robustness** that grows with conditioning and *vanishes at κ=1*; under drift the informed iterate tracks at **exactly `δ(κ−1)`** (the contraction-plus-drift law). The parameter-free both-poles algorithm + lower bound remain C7's open frontier, not gated. Fully deterministic (no RNG); gate unit-tested to fail on tampering |
| AG · third experiment (C5) | ✅ prior PR | **the loop's first fully self-generated experiment**: direction C5 (born from the fifth literature's conjecture, judged *uncertain*, refined *promising* — all this session) proposed its own numerical test, and the system ran it. Pure-Python OGD on two-player quadratic games `F(z)=(μI+γS)z+b`: the **empirical per-step rate equals the closed-form spectral radius in every phase cell** (the OGD map is a scaled rotation), the judge's Banach condition `η<2μ/L²` is proven **identical** to `ρ<1` (modulus = spectral radius, gap <1e-12), the bilinear pole diverges at `√(1+η²γ²)` with quadratically-vanishing excess (continuous-time recurrence), and **averaged play stays bounded while the last iterate wanders** — the C5 dichotomy observed directly. Gate unit-tested to fail on tampered cells |
| AF · evaluate field 5 | ✅ prior PR | the **discover→predict→evaluate→investigate loop closed over the fifth literature**: the online-learning conjectures judged by the in-session sub-agent (C5 fixed-point-of-no-regret-play *uncertain*, C7 curvature→no-regret *uncertain*, and the headline **C6 "conformal calibration is a no-regret play" judged *plausible*** — the system re-derived the Gibbs–Candès adaptive-conformal-inference bridge by analogy); the two uncertain ones refined into gated research directions (C5 *promising*: the last-iterate convergence/recurrence phase boundary; C7 *narrow*: the price-of-robustness question). Evaluation gate now loads every corpus from `multidomain.CORPORA` so it stays in lockstep with the pipeline |
| AE · analogy-web discrimination | ✅ prior PR | the **negative control** for the five-way web: a structurally-broken twin of the online-learning field with *identical vocabulary* but the `CAUSE(premise→pivot→guarantee)` spine removed loses its `PC/2` pivot role and **all** its cross-domain analogies (score collapses 7.0 → 0), while the intact field bridges strongly. Proves the analogy is driven by causal *structure*, not shared functor names — the web discriminates, it isn't promiscuous (positive-vs-negative control, like the grounding riddles) |
| AD · reproducibility + grounding coverage | ✅ prior PR | **solidify**: two whole-pipeline gates. (1) *Grounding coverage* — every result that can enter the unified graph (every corpus in `multidomain.CORPORA` + the real arXiv paper) must pass `grounding.verify`, so no ungrounded/hallucinated-symbol result can be ingested and any future literature is auto-checked. (2) *Determinism* — the unsupervised analogy discovery is run in two separate processes with different `PYTHONHASHSEED`; the discovered roles and analogy pair-set must be identical, so the Epic-N nondeterminism bug class can't silently return. Both gates unit-tested to fail on tampered input |
