...
| Q · conjecture evaluation | ✅ this PR | **closes discover→predict→evaluate**: an in-session Claude Code sub-agent judges each conjecture (no API call); a **deterministic gate** validates the committed judgment + grounds every verdict to a real transfer conjecture. The fixed-point conjecture is judged **plausible** (recovers conformal self-consistency); a capacity conjecture **implausible** (conformal is distribution-free). Gate unit-tested to fail on tampering |
